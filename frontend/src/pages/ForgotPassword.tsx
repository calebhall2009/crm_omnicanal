...
import { Bot, Mail, AlertCircle, ArrowLeft, CheckCircle } from 'lucide-react';
...
export const ForgotPassword: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatus(data.status || 'Te hemos enviado por correo el enlace de restablecimiento de contraseña.');
    } catch (err: any) {
      console.error(err);
    ...
    <div className="min-h-screen bg-[#090b0f] text-gray-100 font-sans flex items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-[#0f111a]/80 border border-gray-800/80 rounded-2xl p-8 shadow-2xl relative z-10 backdrop-blur-sm">
        ...
        <div className="flex flex-col items-center justify-center text-center mb-8">
          ...
          <h2 className="text-2xl font-bold text-white tracking-tight">¿Olvidaste tu contraseña?</h2>
          <p className="text-gray-400 text-xs mt-1">Ingresa tu correo para recibir un enlace de restablecimiento</p>
        </div>
        ...
        <div className="mt-6 flex justify-center">
          <Link to="/login" className="text-xs text-gray-500 hover:text-white flex items-center space-x-1.5 transition">
